 ...
 * артикулом и географией доставки, пока реальные описания не заполнены в админке. */
function fallbackDescription(product: Product): string {
  const article = product.article ? ` Артикул: ${product.article}.` : "";
  return clamp(
    `${product.name}.${article} Подбор по модели техники и артикулу. Доставка по Кыргызстану и в страны СНГ.`,
  );
}
