...
export default async function ProductRedirectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [product, categoryBrandSlugs, directProductCategorySlugs] = await Promise.all([
    getProduct(slug),
  ...
  if (!product) notFound();
  permanentRedirect(getProductHref(product, categoryBrandSlugs, directProductCategorySlugs));
}
