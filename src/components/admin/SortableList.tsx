...
export function SortableList<T>({ items, getId, onReorder, renderItem, className, disabled }: SortableListProps<T>) {
  ...
  if (!mounted || disabled) {
    return (
      <div className={cn("flex flex-col gap-2", className)}>
        {items.map((item) => (
          <div key={getId(item)} className="flex items-center gap-2 rounded-md border border-border bg-card p-2">
  ...
  return <SortableListClient items={items} getId={getId} onReorder={onReorder} renderItem={renderItem} className={className} />;
}

function SortableListClient<T>({ items, getId, onReorder, renderItem, className }: SortableListProps<T>) {
