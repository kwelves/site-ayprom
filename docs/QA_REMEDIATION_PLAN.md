...
**Что остаётся непроверенным.** Два случая в LCP-замерах на телефоне не дали ни одной записи `largest-contentful-paint` — это особенность измерения, а не страницы (в соседних прогонах запись есть). Не покрыт сценарий, где качественная ступень после подмены не падает с ошибкой, а бесконечно ждёт данных: сейчас возврат к стартовой ступени привязан к событию `error`, сторожевого таймера на «зависание без ошибки» нет. Проверки выполнялись на локальном production-сервере; deploy не выполнялся.
