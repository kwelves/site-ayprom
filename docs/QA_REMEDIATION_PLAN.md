...
**Статус реализации.** QA-006 остаётся `Открыт`. Master найден и проверен, пара D5/M5 предварительно утверждена владельцем и подтверждена живым замером. Реализация не начата и ожидает: (1) отдельного разрешения на загрузку двух файлов в Supabase Storage — Hero читает видео оттуда, а не из `public/`; (2) разрешения на изменение кода Hero. Hero, загрузочный экран, `Cache-Control` и остальной код сайта не изменялись; в Supabase Storage ничего не загружалось.
