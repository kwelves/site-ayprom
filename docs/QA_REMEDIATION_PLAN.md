...
**Master отсутствует.** `scripts/generate-hero-video.mjs` ссылается на `public/videos/hero-background-desktop.mp4` и описывает источник как landscape 3840×2160; файла нет ни в рабочем дереве, ни в истории. Существуют только два уже сжатых derivative, которые в соответствии с планом не считаются автоматически утверждёнными.

**Статус реализации.** QA-006 остаётся `Открыт` и не реализуется: обе ступени должны нарезаться из одного master, а изготовление стартовой версии повторным сжатием существующего derivative даёт потерю на потере и отдельно запрещено решением пользователя от 2026-08-27. Реализация ожидает (1) исходный master и (2) последующее визуальное утверждение перехода между ступенями. Снятие `Cache-Control: no-cache` также отложено этим же решением.
