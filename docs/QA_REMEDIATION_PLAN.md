...
**Что не проверено.** LCP отдельно не измерялся. Путь отказа качественной ступени проверен модульно (терминальное состояние, отсутствие повторных попыток, стартовая остаётся видимой), но не живым обрывом сети. Проверка выполнялась на локальном production-сервере; deploy не выполнялся.

**Статус реализации.** QA-006 остаётся `Открыт` до выкладки и замера LCP. Код, тесты и файлы готовы; `Cache-Control` не трогался — Storage по-прежнему отдаёт `no-cache`, в том числе на новой загрузке, где скрипт запрашивал год.
