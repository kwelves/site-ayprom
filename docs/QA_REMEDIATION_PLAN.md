...
| Поле | Значение |
|---|---|
| Статус | Активный источник истины; фазы 0–4 и внеочередная 4A завершены. QA-001…QA-005, QA-012 и QA-013 закрыты; QA-006…QA-011 остаются открытыми |
...
**Статус реализации.** QA-006 остаётся `Открыт`. Master найден и проверен, идёт подготовка материалов: стартовые варианты нарезаются из master напрямую, повторное сжатие существующих derivative не применяется. Реализация по-прежнему ожидает визуального утверждения владельцем: окончательный выбор качества и разрешение на изменение Hero не приняты. Hero, загрузочный экран, `Cache-Control` и остальной код сайта не изменялись; в Supabase Storage ничего не загружалось.
...
8. Когда и в каком порядке миграция QA-013 применяется к production? Локально она проверена полностью, но на production у `anon`/`authenticated` сейчас реально есть TRUNCATE на всех таблицах каталога, и до применения миграции этот доступ сохраняется. Нужно решение владельца по окну применения вместе с остальными неприменёнными миграциями фаз 3–4.
