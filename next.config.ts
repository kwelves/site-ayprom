...
const nextConfig: NextConfig = {
  ...
  experimental: {
    ...
    serverActions: {
      // Транспорт не должен быть жёстче собственной валидации файла.
      //
      // `validateRasterImage` принимает фотографии до 8 МБ (MAX_IMAGE_BYTES),
      // но Server Action по умолчанию обрывает тело на 1 МБ. Из-за этого
      // рассогласования загрузка фото в режиме редактирования товара падала
      // с HTTP 413 ещё до валидации, а администратор видел не понятное
      // «файл слишком большой», а служебное «An error occurred in the Server
      // Components render». Клиентское сжатие (1920 px, JPEG q0.82) спасает
      // не всегда: у детализированных снимков — металл, резьба, фактура —
      // результат остаётся больше мегабайта, и именно каталожные фото
      // деталей попадают в этот случай чаще всего.
      //
      // 10 МБ = 8 МБ файла плюс запас на multipart-обвязку (границы,
      // заголовки частей, служебные поля формы). Верхней границей остаётся
      // валидация: файл крупнее 8 МБ будет отклонён ею с внятным текстом,
      // а не молча оборван транспортом. Режим создания товара этот лимит не
      // затрагивает — там фотографии идут мимо Server Action, напрямую в
      // приватное промежуточное хранилище (QA-004).
      bodySizeLimit: "10mb",
    },
  },
