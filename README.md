...
Локальная verification фазы 1: preflight и production build PASS, smoke 12/12 PASS, полный набор — 43 outcome без unexpected results. Это локальная проверка, не результат запуска GitHub Actions CI.

После фазы 2 состав изменился. Пробы QA-002 и QA-003 убраны из `public/qa-regressions`: браузер не может ни прервать транзакцию на середине, ни отправить набор идентификаторов, которого интерфейс не формирует, поэтому их контракты доказываются в pgTAP (`atomic_product_mutations`, `reorder_contracts`). Вместо них добавлен `admin/product-version-conflict.spec.ts` — он проверяет то, чего pgTAP увидеть не может: видимость и текст отказа при конфликте версий. Проба QA-012 сужена до оставшейся половины (server-side content validation, фаза 3); bounds закрыты и проверяются в `visual_scale_bounds`. Локальный прогон admin-проекта после этих изменений: 14/14 PASS двумя запусками подряд.
