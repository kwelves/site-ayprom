...
vi.mock("framer-motion", () => ({
  ...
  motion: {
    span: ({ initial, animate, exit, transition, ...props }: MotionSpanProps) => (
      <span
        {...props}
        data-testid="hover-border-highlight"
...
describe("HoverBorderGrid", () => {
  it("рисует одну подложку с геометрией и пружиной текущей сетки Каталога", async () => {
    ...
    const firstHighlight = await view.findByTestId("hover-border-highlight");
  ...
  it("синхронизирует подложку без пружинного запаздывания после resize, layout shift и scroll", async () => {
    const view = render(<TestGrid />);
    const grid = view.container.querySelector("[data-hover-border-grid]")!;
    const first = view.getByTestId("one");
    ...
    await view.findByTestId("hover-border-highlight");
    firstRect.left = 70;
    firstRect.top = 90;
  ...
  it("автоматически принимает карточки, добавленные после первого рендера", async () => {
    const view = render(<TestGrid />);
  ...
  it("убирает пространственную анимацию при prefers-reduced-motion", async () => {
